...
def main(args):
    ...
    train_dataset = TxPDataset(KFOLD[args.fold][0] , KFOLD[args.fold][1], transform_train, args, train = True)
    test_dataset = TxPDataset(KFOLD[args.fold][1]  , KFOLD[args.fold][1], transform_test, args, train=False)
    ...
    CONFIG = collections.namedtuple('CONFIG', ['lr', 'logfun', 'verbose_step', 'weight_decay', 'store_dir', 'filter_name', "store_all", "fold"])
    config = CONFIG(args.lr, print, args.verbose_step, args.weight_decay,store_dir,filter_name, False, args.fold)
    ...
    plt = pl.Trainer(max_epochs = args.epoch,num_nodes=args.num_nodes, gpus=args.gpus, val_check_interval = args.val_interval,strategy=DDPPlugin(find_unused_parameters=False),checkpoint_callback = False, logger = False, accelerator='gpu', devices=1)
